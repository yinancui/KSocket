...
void HandleData() {
	...
	if (retval==0) { 
		//break; // Connection has been closed, terminate and go back to draw function
		std::cout << "connection closed.\n";
		return;
	} else if (retval==SOCKET_ERROR) {
		//throw HRException("socket error while receiving.");
		// recv error, terminate and go back to draw function
		std::cout << "failed to receive from server.\n";
		return;
	} else {
		// recv from server successfully, print received frame data and go back to draw function
		//std::cout << "bytes received: " << retval << std::endl;
		tempBuffer[retval] = 0;
