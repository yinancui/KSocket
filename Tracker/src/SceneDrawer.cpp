...
//XnUInt16 nUsers = 15;
char frameData[300] = "";
void HandleConnection(char* frameData);
...
void HandleConnection(char* frameData) {
	...
	//	std::cout << "socket error while receiving from client.\n";
	//	return;
	//} else {
	...
	//	//	throw ROTException("socket error while sending.");
	//	//if (send(hClientSocket, tempBuffer, retval, 0) == SOCKET_ERROR) {
	//	//	std::cout << "socket error while sending to client.\n";
	//	//	return;
	//	//}
	//			
	//	if (send(hClientSocket, frameData, sizeof(frameData), 0) == SOCKET_ERROR) {
	...
	if (send(hClientSocket, frameData, (int)strlen(frameData), 0) == SOCKET_ERROR) {
		std::cout << "error while sending to socket client.\n";
		return;
	}
...
//	retval = recv(hClientSocket, tempBuffer, sizeof(tempBuffer), 0);
//	if (retval==0) { 
//		// Connection has been closed, move on
...
//		std::cout << "bound to port: " << portNumber << std::endl;
//
...
//			throw ROTException("no request from client.");
//		} else {
//			// request accepted
//			std::cout << "accepted.\n";
//			//// Wait for and accept a connection:
//			////HandleConnection(hClientSocket, clientSockAddr); 
//			//std::cout << "Connected with " << GetHostDescription(clientSockAddr) << ".\n";
//
//			char tempBuffer[TEMP_BUFFER_SIZE];
//			int retval;
//			retval = recv(hClientSocket, tempBuffer, sizeof(tempBuffer), 0);
...
//
//	strcpy(frameData, "");
//	//-------- close sockets when one loop is done
...
//
//	std::cout << "done with socket looping.\n";
//	return bSuccess;
//
//}
//
...
//			// good so far, set iRet to 0
//			//iRet = 0;
//			iRet = !RunServer(port);
//		}
//		else
//		{
//			std::cerr << "required version not supported!";
//		}
//
